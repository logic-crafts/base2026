-- Preserve every legacy receipt row before widening the reviewer contract.
...
CREATE TABLE editorial_publication_receipts (
  ...
  reviewer TEXT NOT NULL CHECK (reviewer IN ('sol-max', 'gpt-6-astra')),
...
SELECT slug, revision, payload_sha256, published_at, updated_at, reviewer, reviewed_at, recorded_at
  FROM editorial_publication_receipts_legacy_0007;
