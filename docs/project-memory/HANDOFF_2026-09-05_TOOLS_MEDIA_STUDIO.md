...
## Exact builder integration — combined candidate

The combined PR57/58 candidate adds a reviewed repository-media path in
`scripts/build-base2026-cloudflare-release.py`. Only the four exact names in
the pinned manifest map to `static/assets/tools-studio/`; source byte counts,
SHA-256 hashes and provenance remain bound to that review. All other binary
preservation checks stay in force. Missing or modified reviewed media fails
closed rather than being silently admitted through a directory glob.

Do not use the former generic generated-file mapping for these binaries:
it records an empty source hash and a changed binary, which correctly fails
the preservation gate. The combined candidate records repository provenance
separately and verifies exact output bytes. Never disguise media as text or
disable the general binary check. This source correction does not itself
establish a deployed release.
